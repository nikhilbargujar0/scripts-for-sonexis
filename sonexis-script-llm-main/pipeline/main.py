...
def _speaker_pair_alignment_report(pair: SpeakerPairAudio, max_mismatch_s: float) -> Dict:
    """Report shared-timeline assumptions for separated speaker files.

    Separate-speaker recordings normally start at the same session clock. We do
    not invent offsets from isolated voices; instead we record explicit zero
    offsets and validate duration mismatch.
    """
    durations = {
    ...
    return {
        ...
        "confidence": 0.9 if mismatch <= max_mismatch_s else 0.55,
